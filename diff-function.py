...
def get_arguments():
    ...
    parser.add_argument("--depth", default = 0, required=False, 
                        ...
                        """, type=int)
    parser.add_argument("--no-combinations", required=False, 
                        help=
                        """
                        Do not compute the N(N-1)/2 combinations for the N supplied commits, or the
                        N commits generated by using the '--depth' flag. Instead, compare the
...
def validate_arguments(arguments):
    ...
    if arguments.no_combinations and not arguments.depth:
        sys.stderr.write("""When using the '--no-combinations flag, you must also use the '--depth' flag. 
                         Also, you must supply only one commit.""")
        sys.exit(1)
...
def changes(repo_dir, commits, arguments):
    ...
    g = git.Git(repo_dir)
    commit_pairs = []
    if arguments.no_combinations:
        commit_pairs = [(commits[0], j) for j in commits if commits[0] != j]
    else:
